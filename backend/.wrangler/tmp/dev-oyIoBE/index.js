...
// .wrangler/tmp/bundle-vAb7zu/strip-cf-connecting-ip-header.js
  ...
  async fetch(request, env, ctx) {
    ...
    try {
      if (method === "POST" && url.pathname === "/api/ieqi") {
        ...
        if (typeof temperature !== "number" || typeof humidity !== "number" || typeof light !== "number" || typeof ieqi !== "number" || !device_id) {
        ...
        const query = `
          ...
          VALUES (?, ?, ?, ?, ?)
        `;
        const result = await env.DB.prepare(query).bind(device_id, temperature, humidity, light, ieqi).run();
        if (result.success) {
...
// .wrangler/tmp/bundle-vAb7zu/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-vAb7zu/middleware-loader.entry.ts
